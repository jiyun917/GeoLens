...
class AutoGraphBuilder:
    ...
    def _build_one_workflow(
        ...
        for step_number, (text, meta) in enumerate(procedural_sorted, start=1):
            ...
            visual_sig = self._extract_visual_signature(chunk_index, all_chunks)
    ...
    def _extract_visual_signature(
        ...
        """Aggregate ui_elements and likely dialog names from nearby ui_description chunks."""
        ...
        # Fallback: also fold in the action step's own ui_elements if present
        return {
            "expected_dialog": expected_dialog,
            "expected_ui_elements": ui_elements[:10],
            "screenshot_keywords": list(dict.fromkeys(keywords))[:10],
        }
    ...
    def _summarize_titles(self, step_texts: List[str]) -> List[str]:
        ...
        try:
            ...
            print(f"[AUTO_GRAPH] title summarization failed: {e}")
        return fallback
